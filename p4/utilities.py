...
def cross_validate_custom_forest(nfolds, seed, x, y, clf):
    '''
        Customize cross validation routine,
        with access to y_pred of each fold.
    '''
    y_pred = np.zeros(y.shape)
    trn_rmse = np.zeros((nfolds, ))
    tst_rmse = np.zeros((nfolds, ))
    oob_error = np.zeros((nfolds, ))
        
    folds = KFold(n_splits=nfolds, shuffle=False, random_state=seed).split(y)
    ...
    for i, (trn_idx, tst_idx) in enumerate(folds):
        ...
        clf.fit(x_trn, y_trn)
        y_pred[tst_idx] = clf.predict(x_tst)
        trn_rmse[i] = np.sqrt(mean_squared_error(y_trn, clf.predict(x_trn)))
        oob_error[i]=1-clf.oob_score_
    return y_pred, np.mean(trn_rmse), np.mean(oob_error)
...
def evaluate_results(trn_rmse, tst_rmse, y_true, y_pred, filename):
    ...
    ax1.scatter(y_true.index, y_true, s=3)
    ax1.scatter(y_true.index, y_pred, s=3)
    ax1.set_xlabel("Record Index")
    ax1.set_ylabel("Size of backup (GB)")
    ax1.legend(['y_true', 'y_pred'])

    ax2 = fig.add_subplot(212)
    ax2.scatter(y_true.index, y_pred-y_true, s=3)
    ax2.set_xlabel("Record Index")
    ax2.set_ylabel("Size of backup (GB)")
    ax2.legend(['Residual'])
